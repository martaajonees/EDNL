...
public: 
  ...
  Apo<T>& operator=(const Apo<T>& A); // asignacion de apo 
  ~Apo();                              // destructor
private: 
  typedef size_t nodo; // indice del vector 
                    // entre 0 y maxNodos-1 
  T* nodos;      // vector de nodos 
  size_t maxNodos;  // tamanno del vector 
  size_t numNodos;  // ultimo nodo del árbol
...
inline void Apo <T>::insertar(const T& e) {
  assert(numNodos< maxNodos);   // Apono lleno.
  nodos[numNodos] = e; 
  if(++numNodos> 1) 
...
inline void Apo<T>::suprimir() { 
  assert(numNodos > 0); //APO no vacio
  if(--numNodos > 0){ //APO no queda vacio
    nodos[0] = nodos [numNodos];
    if(numNodos > 1) //Quedan dos o mas elementos
      hundir(0);
  }
}
