...
class Abb {
    public:
        ...
        void borrado(const T &e); // Actividad 1
        ~Abb(); // destructor
    private:
        struct arbol {
            T elto;
        ...
        T borrarMin();
        Abb<T>& busqueda(const T& e) const; //Actividad 1
};
...
template <typename T>
Abb<T>& Abb<T>::busqueda(const T& e) const{
     if (r == nullptr) // Árbol vacío, e no encontrado.
        return *this;
    else if (e < r->elto) // Buscar en subárbol izqdo.
        return r->izq.buscar(e);
...
void borrado(const T &e){
    Abb<T> A = A.buscar(e); // Busco el subarbol que quiero eliminar
    const T &aux = e;
    return borradoRec(A,e, aux);
}
