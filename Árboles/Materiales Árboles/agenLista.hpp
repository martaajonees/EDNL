...
template <typename T> class Agen {
    public:
        ...
        const T& elemento(nodo n); // Lectura/escritura en Agenno-const
        nodo raiz() const;
...
void Agen<T>::insertarHijoIzqdo(nodo n, const T& e)
{
    ...
    for (hizqdo = 1; nodos[hizqdo].padre != NODO_NULO; hizqdo++) nodos[hizqdo].elto = e;
    nodos[hizqdo].padre = n;
    // Insertar el nuevo nodo al inicio de la lista de hijos de n.
    listaenla<nodo>& Lh = nodos[n].hijos; // Lista de hijos.
...
void Agen<T>::insertarHermDrcho(nodo n, const T& e)
{
    ...
    // Añadir el nuevo nodo en la primera celda libre.
    for (hedrcho = 1; nodos[hedrcho].padre != NODO_NULO; hedrcho++) nodos[hedrcho].elto = e;
...
void Agen<T>::eliminarHijoIzqdo(nodo n)
{
    ...
    nodos[hizqdo].hijos.fin()); // de n es una hoja.// Eliminar hijo izqdo. de n.
...
template <typename T>
inline const T& Agen<T>::elemento(nodo n)
{
    assert(numNodos > 0); // Árbol no vacío.
    assert(n >= 0 && n <= maxNodos-1); // n es una celda del vector
    assert(n == 0 || nodos[n].padre != NODO_NULO); // que está ocupada
...
inline typename Agen<T>::nodo Agen<T>::hijoIzqdo(nodo n) const{
    ...
    assert(n >= 0 && n <= maxNodos-1); // n es una celda del vector
    assert(n == 0 || nodos[n].padre != NODO_NULO); // que está ocupada.
    listaenla<nodo>& Lh = nodos[n].hijos;
    if (Lh.primera() != Lh.fin()) /* Lista no vacía.*/ return Lh.elemento(Lh.primera());
...
inline typename Agen<T>::nodo Agen<T>::hermDrcho(nodo n) const{
    ...
    assert(n >= 0 && n <= maxNodos-1); // n es una celda del vector
    assert(n == 0 || nodos[n].padre != NODO_NULO); // que está ocupada.
    if (n == 0) /* n es la raíz.*/ return NODO_NULO;
    else
    {
        listaenla<nodo>& Lhp = nodos[nodos[n].padre].hijos; // Lista de hijos del padre.
        listaenla<nodo>::posicion p = Lhp.primera();
...
{
    for (nodo n = 0; n <= maxNodos-1; n++) /* Copiar el vector.*/nodos[n] = A.nodos[n];
}
