...
template <typename T> class Abin {
    ...
    int profundidadRec(typename Abin<T>::nodo n);//Ejercicio 5
    int numAltura(typename Abin<T>::nodo n);//Ejercicio 5
    void eliminarRaiz();
...
//Cabecera: int profundidadRec(Abin<T>& A, typename Abin<T>::nodo n, typename Abin<T>::nodo r, int deep)
//Precondición: Recibe un árboly el nodo del que se quiere saber su profundidad 
template <typename T>
int Abin<T>::profundidadRec(typename Abin<T>::nodo n){
    if(n==NODO_NULO){ // Si el padre es nulo, entonces n es la raíz
        return -1;
    }else{
        return 1 + profundidadRec(n->padre);// Se sigue subiendo en el árbol
    }
}

//Recibe la raiz
template<typename T>
int Abin<T>::numAltura(typename Abin<T>::nodo n){
    if(n==NODO_NULO){
        return -1;
    }else{
        int Drch=numAltura(n->hder);
        int Izq=numAltura(n->hizq);
        return 1+ max(Drch,Izq);
    }
}
#endif